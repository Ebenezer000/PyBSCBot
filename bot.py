...
TOKEN = '1808975879:AAEOHuDcgZ--98iHqjIlmd31pVIgvcWcIA8'
...
def rain(update, context):
    ...
    if chat.type == Chat.PRIVATE:
        ...
        if password == "":
           ...
           try:
                command = context.args[2].upper()
                ramount =  int(float(context.args[0]))
                rusers = int(context.args[1])
                minreq = ramount*10**18
...
def Address(update: Update, context: CallbackContext) -> str:
    ...
    if chat.type == Chat.PRIVATE:
        ...
        if password == "":
            ...
            context.bot.send_message(chat_id = sender, text = ('⚠ You can only have full access to this service by signing up'),reply_markup=first_menu_keyboard())
        else:
            context.bot.send_message(chat_id = chat_id, text = 'Hi '+username+'\nYour Deposit Address:\n'+address+'\nNOTE:\nThis address can only recieve TTK, BNB, BUSD and USDT(BEP20)')
...
def menu(update: Update, context: CallbackContext) -> str:
     ...
     chat_id = update.effective_chat.id
     if chat.type == Chat.PRIVATE:
         context.bot.send_message(chat_id = chat_id, text = main_menu_message(),
                              reply_markup=main_menu_keyboard())
     elif chat.type in [Chat.GROUP, Chat.SUPERGROUP]:
         y = 2+1
     else:
...
def Prices(update, context):
    ...
    chat = update.effective_chat
    bnbpage = requests.get('https://coinmarketcap.com/currencies/binance-coin/')
    btcpage = requests.get('https://coinmarketcap.com/currencies/bitcoin/')
    ethpage = requests.get('https://coinmarketcap.com/currencies/ethereum')
    bnbtree = html.fromstring(bnbpage.content)
    btctree = html.fromstring(btcpage.content)
    ethtree = html.fromstring(ethpage.content)
    _bnbprice = str(bnbtree.xpath('//div[@class="priceValue "]/text()'))
    _btcprice = str(btctree.xpath('//div[@class="priceValue "]/text()'))
    _ethprice = str(ethtree.xpath('//div[@class="priceValue "]/text()'))
    bnb_price = _bnbprice.strip(" [''] ")
    btc_price = _btcprice.strip(" [''] ")
    eth_price = _ethprice.strip(" [''] ")
    if chat.type == Chat.PRIVATE:
...
def balance(update, context):
    ...
    if chat.type == Chat.PRIVATE:
        ...
        if signed == "":
            ...
            ttk_balance = str(round(float(ttkcontract.functions.balanceOf(address).call()/10**18)), 4)
            bnb_bal = str(round(float(w3.eth.get_balance(address)/10**18)), 4)
            busd_bal = str(round(float(busdcontract.functions.balanceOf(address).call()/10**18)), 4)
            usdt_bal = str(round(float(usdtcontract.functions.balanceOf(address).call()/10**18)), 4)
        ...
        if password == "":
            ...
            busd_bal = str(round(float(busdcontract.functions.balanceOf(address).call()/10**18)), 4)
            usdt_bal = str(round(float(usdtcontract.functions.balanceOf(address).call()/10**18)), 4)
            context.bot.send_message(chat_id = chat_id, text = 'Your account Balace has been sent to you privately')
            context.bot.send_message(chat_id = sender, text = ('Your Account Balance today is: \n'+ttk_balance+' TTK,\n'+bnb_bal+' BNB,\n'+busd_bal+' BUSD,\n'+usdt_bal+' USDT'))
    else:
        pass
...
def main():
    ...
    updater.start_webhook(listen="0.0.0.0",
                          port=int(PORT),
                          url_path=TOKEN,
                          webhook_url='https://ezentest2.herokuapp.com/' + TOKEN)
